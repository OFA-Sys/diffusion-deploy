...
class Engine():
    ...
    def build(self, onnx_path, fp16, input_profile=None, enable_preview=False):
        ...
        engine = engine_from_network(network_from_onnx_path(onnx_path),
                                     config=CreateConfig(
                                         fp16=fp16,
                                         profiles=[p],
    ...
    def allocate_buffers(self, shape_dict=None, device='cuda'):
        for idx in range(trt_util.get_bindings_per_profile(self.engine)):
            ...
            tensor = torch.empty(
                tuple(shape), dtype=torch_type_tensor.dtype).to(device=device)
...
class LMSDiscreteScheduler():
    ...
    def scale_model_input(self, sample: torch.FloatTensor, idx, *args,
    ...
    def configure(self):
        ...
        self.lms_coeffs = []
        self.latent_scales = [
            1. / ((sigma**2 + 1)**0.5) for sigma in self.sigmas
        ]
    ...
    def step(self, output, latents, idx, timestep):
        ...
        prev_sample = latents + sum(
            coeff * derivative for coeff, derivative in zip(
...
class DPMScheduler():
    ...
    def scale_model_input(self, sample: torch.FloatTensor, *args,
                          **kwargs) -> torch.FloatTensor:
        return sample
    ...
    def configure(self):
        ...
        for step_index in range(self.num_inference_steps):
            ...
            prev_timestep = 0 if step_idx == len(
                self.timesteps) - 1 else self.timesteps[step_idx + 1]

            self.dpm_solver_first_order_coefs_precompute(
    ...
    def dpm_solver_first_order_coefs_precompute(self, timestep, prev_timestep):
        ...
        if self.algorithm_type == "dpmsolver++":
            ...
            self.first_order_second_coef.append(alpha_t *
                                                (torch.exp(-h) - 1.0))
    ...
    def multistep_dpm_solver_second_order_coefs_precompute(
        ...
        if self.algorithm_type == "dpmsolver++":
            ...
            if self.solver_type == "midpoint":
                ...
                self.second_order_second_coef.append(
    ...
    def multistep_dpm_solver_third_order_coefs_precompute(
        ...
        if self.algorithm_type == "dpmsolver++":
            ...
            self.third_order_second_coef.append(alpha_t *
                                                (torch.exp(-h) - 1.0))
    ...
    def set_timesteps(self, num_inference_steps):
        ...
        timesteps = (np.linspace(0, self.num_train_timesteps - 1,
                                 num_inference_steps +
                                 1).round()[::-1][:-1].copy().astype(np.int32))
        ...
        if self.algorithm_type == "dpmsolver++":
            ...
            if self.thresholding:
                ...
                dynamic_max_val = torch.maximum(
                    ...
                    torch.ones_like(dynamic_max_val).to(
                        dynamic_max_val.device),
                )[(..., ) + (None, ) * (x0_pred.ndim - 1)]
                x0_pred = torch.clamp(x0_pred, -dynamic_max_val,
    ...
    def multistep_dpm_solver_second_order_update(self, idx, model_output_list,
                                                 timestep_list, prev_timestep,
                                                 sample):
        t, s0, s1 = prev_timestep, timestep_list[-1], timestep_list[-2]
        ...
        if self.algorithm_type == "dpmsolver++":
            # See https://arxiv.org/abs/2211.01095 for detailed derivations
            if self.solver_type == "midpoint":
                x_t = (first_coef * sample - second_coef * D0 -
                       third_coef * D1)
            elif self.solver_type == "heun":
                x_t = (first_coef * sample - second_coef * D0 +
                       third_coef * D1)
    ...
    def multistep_dpm_solver_third_order_update(self, idx, model_output_list,
                                                timestep_list, prev_timestep,
                                                sample):
    ...
    def step(self, output, latents, step_index, timestep):
        ...
        if self.solver_order == 1 or self.lower_order_nums < 1 or lower_order_final:
            ...
            prev_sample = self.multistep_dpm_solver_second_order_update(
                step_index, self.model_outputs, timestep_list, prev_timestep,
                latents)
        else:
            timestep_list = [
                self.timesteps[step_index - 2], self.timesteps[step_index - 1],
...
def save_image(images, image_path_dir, image_name_prefix):
    ...
    for i in range(images.shape[0]):
        image_path = os.path.join(
            image_path_dir, image_name_prefix + str(i + 1) + '-' +
